...
export const banAppealMessage = async (fields: ModalSubmitFields) => {
  ...
  const dob = `**Date of Ban:** ${escapeDiscordMarkdown(fields.getTextInputValue('appeal_dob_input'))}\n`;
  const s = `**Staff who banned you:** ${escapeDiscordMarkdown(fields.getTextInputValue('appeal_staff_input'))}\n`;
  const r = `**Why you think you should be unbanned:** ${escapeDiscordMarkdown(fields.getTextInputValue('appeal_reason_input'))}\n`;
