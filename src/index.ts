...
client.on("messageReactionAdd", (reaction, user) =>
  handleRulesReactionAdd(reaction, user, client),
);
client.on("messageReactionRemove", (reaction, user) =>
  handleRulesReactionRemove(reaction, user, client),
);
client.on("messageReactionAdd", (reaction, user) =>
  handleReactionRoleAdd(reaction, user, client),
);
client.on("messageReactionRemove", (reaction, user) =>
...
client.on(messageDeleteEvent.name, (message) =>
  messageDeleteEvent.execute(message, client),
);

// register guild lifecycle events
client.on(guildCreateEvent.name, (guild) =>
  guildCreateEvent.execute(guild, client),
);
client.on(guildDeleteEvent.name, (guild) =>
  guildDeleteEvent.execute(guild, client),
);

// register channel lifecycle events
client.on(channelDeleteEvent.name, (channel) =>
  channelDeleteEvent.execute(channel, client),
);
...
client.once("clientReady", async () => {
  ...
  (client as typeof client & { statusManager: StatusManager }).statusManager =
    statusManager;
  ...
  if (!IS_DEV) {
    ...
    if (apiConnector.isConnectedToAPI()) {
      ...
      enhancedLogger.warn(
        "API registration failed, continuing without it",
        LogCategory.API,
      );
    }
  } else {
...
async function main() {
  try {
    ...
    for (const config of botConfigs) {
      try {
        await rest.put(
          Routes.applicationGuildCommands(CLIENT, config.guildId),
          {
            body: commands,
          },
        );
