...
const mee6 = new SlashCommandSubcommandBuilder()
  ...
  .addBooleanOption(option =>
    option.setName('overwrite').setDescription(tl.overwriteOption).setRequired(false),
  )
  .addBooleanOption(option =>
    option.setName('dry-run').setDescription(tl.dryRunOption).setRequired(false),
  );
...
const csv = new SlashCommandSubcommandBuilder()
  ...
  .addAttachmentOption(option =>
    option.setName('file').setDescription(tl.attachmentOption).setRequired(true),
  )
  .addBooleanOption(option =>
    option.setName('overwrite').setDescription(tl.overwriteOption).setRequired(false),
  )
...
const importStatus = new SlashCommandSubcommandBuilder()
  .setName('status')
  .setDescription(tl.statusDescrp);

const history = new SlashCommandSubcommandBuilder()
  .setName('history')
  .setDescription(tl.historyDescrp);

const cancel = new SlashCommandSubcommandBuilder()
  .setName('cancel')
  .setDescription(tl.cancelDescrp);
