...
export async function channelsHandler(_client: Client, interaction: ChatInputCommandInteraction<CacheType>) {
  ...
  try {
    ...
    enhancedLogger.error('Failed to fetch channel analytics', error as Error, LogCategory.COMMAND_EXECUTION, {
      guildId,
    });
    await interaction.reply({
