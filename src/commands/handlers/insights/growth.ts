...
export async function growthHandler(_client: Client, interaction: ChatInputCommandInteraction<CacheType>) {
  ...
  try {
    ...
    const embed = buildGrowthEmbed(snapshots, days);
    await interaction.reply({ embeds: [embed] });
  } catch (error) {
    enhancedLogger.error('Failed to fetch growth analytics', error as Error, LogCategory.COMMAND_EXECUTION, {
      guildId,
    });
    await interaction.reply({
