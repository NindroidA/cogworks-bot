...
import { enhancedLogger, handleInteractionError, lang, LogCategory } from '../../../utils';
...
const fieldDraftCache = new Map<string, {
    id: string;
    label: string;
    style: string;
    required: string;
...
export async function handleAddFieldModal(interaction: ModalSubmitInteraction, typeId: string): Promise<void> {
    try {
        ...
        const fieldPlaceholder = interaction.fields.getTextInputValue('field_placeholder')?.trim() || undefined;
        ...
        enhancedLogger.info(
            `Field added to ticket type: ${typeId}`,
            LogCategory.COMMAND_EXECUTION,
            { guildId, typeId, fieldId }
        );
 ...
 */
async function showEditFieldSelect(interaction: ButtonInteraction, ticketType: CustomTicketType): Promise<void> {
    const fields = ticketType.customFields || [];
 ...
 */
async function showDeleteFieldSelect(interaction: ButtonInteraction, ticketType: CustomTicketType): Promise<void> {
    const fields = ticketType.customFields || [];
 ...
 */
async function showReorderInterface(interaction: ButtonInteraction, ticketType: CustomTicketType): Promise<void> {
    const fields = ticketType.customFields || [];
 ...
 */
async function showModalPreview(interaction: ButtonInteraction, ticketType: CustomTicketType): Promise<void> {
    const fields = ticketType.customFields || [];
 ...
 */
async function handleDeleteField(interaction: StringSelectMenuInteraction, ticketType: CustomTicketType): Promise<void> {
 ...
 */
async function handleMoveField(interaction: ButtonInteraction, direction: 'up' | 'down', fieldIndex: number, ticketType: CustomTicketType): Promise<void> {
    const fields = ticketType.customFields || [];
    ...
    try {
        ...
        switch (action) {
            ...
            default:
                ...
                if (moveUpMatch) {
                    const fieldIndex = parseInt(moveUpMatch[1]);
                    await handleMoveField(interaction, 'up', fieldIndex, ticketType);
                } else if (moveDownMatch) {
                    const fieldIndex = parseInt(moveDownMatch[1]);
 ...
 */
export async function handleFieldSelectMenu(interaction: StringSelectMenuInteraction, action: string, typeId: string): Promise<void> {
    try {
        const guildId = interaction.guildId!;
        const typeRepo = AppDataSource.getRepository(CustomTicketType);
