...
export async function userRestrictHandler(interaction: ChatInputCommandInteraction): Promise<void> {
	try {
		...
		await showRestrictionsConfigurator(interaction, guildId, targetUser, ticketTypes, restrictionRepo);
...
async function handleSingleTypeToggle(
	...
	restrictionRepo: typeof AppDataSource extends { getRepository: (entity: typeof UserTicketRestriction) => infer R } ? R : never
): Promise<void> {
