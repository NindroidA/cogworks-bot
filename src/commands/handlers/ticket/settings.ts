...
import { Colors, E, enhancedLogger, lang, LANGF, LogCategory } from '../../../utils';
...
export async function settingsHandler(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    if (!guildId) {
        enhancedLogger.warn('Settings handler: guild not found', LogCategory.COMMAND_EXECUTION, { userId: interaction.user.id });
    ...
    enhancedLogger.debug(`Command: /ticket settings ${setting}=${enabled}`, LogCategory.COMMAND_EXECUTION, { userId: interaction.user.id, guildId, setting, enabled, typeId });
    ...
    if (!ticketConfig) {
        enhancedLogger.warn('Settings handler: ticketConfig not found', LogCategory.COMMAND_EXECUTION, { userId: interaction.user.id, guildId });
        await interaction.reply({
    ...
    if (setting === 'admin-only-mention') {
        ...
        enhancedLogger.info(`Setting updated: admin-only-mention=${enabled}`, LogCategory.COMMAND_EXECUTION, { userId: interaction.user.id, guildId, setting, enabled });
    ...
    if (setting === 'ping-on-create') {
        ...
        if (!typeId) {
            enhancedLogger.warn('Settings handler: type required for ping-on-create', LogCategory.COMMAND_EXECUTION, { userId: interaction.user.id, guildId });
            await interaction.reply({
        ...
        if (LEGACY_TYPES.includes(typeId as LegacyType)) {
            ...
            if (!customType) {
                enhancedLogger.warn(`Settings handler: type '${typeId}' not found`, LogCategory.COMMAND_EXECUTION, { userId: interaction.user.id, guildId, typeId });
                await interaction.reply({
        ...
        enhancedLogger.info(`Setting updated: ping-on-create for '${typeId}'=${enabled}`, LogCategory.COMMAND_EXECUTION, { userId: interaction.user.id, guildId, setting, typeId, enabled });
