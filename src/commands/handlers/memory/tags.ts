...
import { MemoryConfig, MemoryTag, MemoryTagType } from '../../../typeorm/entities/memory';
import {
    Colors,
    E,
    lang,
...
async function handleAddTag(interaction: ChatInputCommandInteraction, guildId: string, forumChannelId: string) {
...
async function handleAddTagSubmit(interaction: ModalSubmitInteraction, guildId: string, forumChannelId: string) {
...
async function handleEditTag(interaction: ChatInputCommandInteraction, guildId: string, forumChannelId: string) {
...
async function showEditModal(interaction: StringSelectMenuInteraction, guildId: string, forumChannelId: string) {
    ...
    if (!tag) {
        await interaction.reply({ content: `${E.error} ${tl.tags.edit.tagNotFound}`, flags: [MessageFlags.Ephemeral] });
        return;
    }
...
async function handleEditTagSubmit(interaction: ModalSubmitInteraction, tagId: number, guildId: string, forumChannelId: string) {
    await interaction.deferReply({ flags: [MessageFlags.Ephemeral] });
...
async function handleRemoveTag(interaction: ChatInputCommandInteraction, guildId: string, forumChannelId: string) {
