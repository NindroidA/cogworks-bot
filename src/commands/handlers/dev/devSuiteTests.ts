...
const onboardingCompletionRepo =
  AppDataSource.getRepository(OnboardingCompletion);
...
async function runSmokeTest(
  ...
  if (ticketConfig) {
    ...
    results.push({
      ...
      details: ticketConfig.slaEnabled
        ? `target: ${ticketConfig.slaTargetMinutes}min`
        : "",
      issues,
    });
    ...
    results.push({
      ...
      details: ticketConfig.smartRoutingEnabled
        ? `${ruleCount} rules, strategy: ${ticketConfig.routingStrategy}`
        : "",
      issues: [],
    });
  } else {
  ...
  if (appConfig) {
    results.push({
      ...
      details: appConfig.enableWorkflow
        ? `${appConfig.workflowStatuses?.length ?? 0} statuses`
        : "",
      issues: [],
    });
  } else {
    ...
    try {
      ...
      for (const field of verifyFields) {
        ...
        if (expectedStr !== actualStr) {
          mismatches.push(
            `${field}: expected ${expectedStr}, got ${actualStr}`,
          );
        }
      }
    ...
    try {
      ...
      if (stat.isDirectory()) {
        ...
        const indexFile = files.find(
          (f) => f.startsWith("index.") && !target.fileFilter,
        );
        if (indexFile) {
          const indexContent = fs.readFileSync(
            path.join(targetPath, indexFile),
            "utf-8",
          );
          routerHasAdminGate =
            indexContent.includes("requireAdmin") ||
            indexContent.includes("requireBotOwner");
        }
        ...
        for (const file of files) {
          ...
          if (hasAdmin && hasOwner)
            checkType = "requireAdmin + requireBotOwner";
          ...
          const coveredByRouter =
            routerHasAdminGate && !hasAdmin && !hasOwner && !isPublic;
          ...
          results.push({
            ...
            hasPermissionCheck:
              hasAdmin || hasOwner || isPublic || coveredByRouter,
...
function formatPermissionsEmbed(
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `**${withCheck}** with checks, **${withoutCheck}** without checks`,
    )
...
export async function handleMasterTest(
  ...
  try {
    ...
    enhancedLogger.warn(
      "devSuiteWorkflows not available, skipping populate phase",
      LogCategory.COMMAND_EXECUTION,
    );
  }
  ...
  if (handleScaffoldAll) {
    try {
      ...
      const configuredCount = preCheck.filter(
  ...
  if (handlePopulate) {
    masterResults.push({
      ...
      warnings: 1,
      details: [
        "Populate available but requires interactive context. Run populate separately if needed.",
      ],
    });
    totalWarnings++;
  } else {
  ...
  await interaction.editReply(
    "Running master test... Phase 3/7: Smoke test...",
  );
  try {
    const smokeResults = await runSmokeTest(client, guildId);
    const configured = smokeResults.filter(
      (r) => r.status === "configured",
    ).length;
    const withIssues = smokeResults.filter((r) => r.issues.length > 0).length;
    const notConfigured = smokeResults.filter(
  ...
  // Phase 4: Regression
  await interaction.editReply(
    "Running master test... Phase 4/7: Regression tests...",
  );
  try {
    ...
    masterResults.push({
      ...
      failed,
      warnings: 0,
      details: regressionResults.map(
        (r) => `${r.passed ? "PASS" : "FAIL"} ${r.system}: ${r.details}`,
      ),
    });
    totalPassed += passed;
    totalFailed += failed;
  ...
  await interaction.editReply(
    "Running master test... Phase 6/7: Collecting results...",
  );
  ...
  const summaryEmbed = new EmbedBuilder()
    ...
    .setColor(
      totalFailed > 0 ? 0xff0000 : totalWarnings > 0 ? 0xffaa00 : 0x00ff00,
    )
    .setTimestamp();
