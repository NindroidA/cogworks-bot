...
export const statusHandler = async (client: Client, interaction: ChatInputCommandInteraction) => {
	try {
		...
		const logChannel = config.logChannelId ? 
			await interaction.guild!.channels.fetch(config.logChannelId).catch(() => null) : null;

		const embed = new EmbedBuilder()
			.setColor(config.enabled ? '#00FF00' : '#FF0000')
			.setTitle(tl.status.title)
			.addFields(
				{ name: 'Status', value: config.enabled ? tl.status.statusEnabled : tl.status.statusDisabled, inline: true },
				{ name: 'Channel', value: channel ? `<#${channel.id}>` : tl.status.channelNotFound, inline: true },
				{ name: 'Action Type', value: config.actionType, inline: true },
				{ name: 'Grace Period', value: `${config.gracePeriodSeconds}s`, inline: true },
				{ name: 'Smart Detection', value: config.enableSmartDetection ? tl.status.smartOn : tl.status.smartOff, inline: true },
				{ name: 'Log Channel', value: logChannel ? `<#${logChannel.id}>` : tl.status.logNone, inline: true }
			);
		...
		if ((config.whitelistedRoles?.length || 0) > 0 || (config.whitelistedUsers?.length || 0) > 0) {
			...
			if ((config.whitelistedRoles?.length || 0) > 0) {
				const rolesList = config.whitelistedRoles!
					.map(roleId => `<@&${roleId}>`)
					.join(', ');
			...
			if ((config.whitelistedUsers?.length || 0) > 0) {
				const usersList = config.whitelistedUsers!
					.map(userId => `<@${userId}>`)
					.join(', ');
