...
export const detectionHandler = async (client: Client, interaction: ChatInputCommandInteraction) => {
	try {
		...
		const embed = new EmbedBuilder()
			...
			.addFields(
				{ name: tl.detection.enabled, value: enabled ? tl.detection.yes : tl.detection.no, inline: true },
				{ name: tl.detection.minAccountAge, value: tl.detection.days.replace('{0}', config.minAccountAgeDays.toString()), inline: true },
				{ name: tl.detection.minMembership, value: tl.detection.minutes.replace('{0}', config.minMembershipMinutes.toString()), inline: true },
				{ name: tl.detection.minMessages, value: `${config.minMessageCount}`, inline: true },
				{ name: tl.detection.requireVerification, value: config.requireVerification ? tl.detection.yes : tl.detection.no, inline: true }
			);
