...
export async function startComprehensiveWizard(
  ...
  if (
    existingConfigs &&
    (existingConfigs.existingBotConfig ||
      existingConfigs.existingTicketConfig ||
      existingConfigs.existingApplicationConfig ||
      existingConfigs.existingAnnouncementConfig ||
      existingConfigs.existingBaitChannelConfig)
  ) {
    const alreadyConfigured = [];
    if (existingConfigs.existingBotConfig?.enableGlobalStaffRole)
      alreadyConfigured.push('• Global Staff Role');
    if (existingConfigs.existingTicketConfig && existingConfigs.existingArchivedTicketConfig)
      alreadyConfigured.push('• Ticket System');
    if (
      existingConfigs.existingApplicationConfig &&
      existingConfigs.existingArchivedApplicationConfig
    )
    ...
    if (existingConfigs.existingBaitChannelConfig)
      alreadyConfigured.push('• Bait Channel (Anti-Bot)');

    const toBeConfigured = [];
    if (!existingConfigs.existingBotConfig?.enableGlobalStaffRole)
      toBeConfigured.push('• Global Staff Role (optional)');
    if (!(existingConfigs.existingTicketConfig && existingConfigs.existingArchivedTicketConfig))
      toBeConfigured.push('• Ticket System (optional)');
    if (
      !(
        existingConfigs.existingApplicationConfig &&
        existingConfigs.existingArchivedApplicationConfig
      ...
      toBeConfigured.push('• Application System (optional)');
    if (!existingConfigs.existingAnnouncementConfig)
      toBeConfigured.push('• Announcement System (optional)');
    if (!existingConfigs.existingBaitChannelConfig)
      toBeConfigured.push('• Bait Channel System (optional)');
  ...
  const reply =
    'editReply' in interaction && interaction.deferred
...
async function handleStaffRoleStep(
  interaction: MessageComponentInteraction,
  state: ComprehensiveWizardState,
  client: Client,
) {
...
async function handleStaffRoleSelection(
  interaction: MessageComponentInteraction,
  state: ComprehensiveWizardState,
  client: Client,
) {
...
async function handleTicketSystemStep(
  interaction: MessageComponentInteraction,
  state: ComprehensiveWizardState,
  client: Client,
) {
  // Check if ticket system is already configured
  const ticketAlreadyConfigured =
    state.existingConfigs?.ticketConfig && state.existingConfigs?.archivedTicketConfig;
...
async function handleTicketChannelSelect(
  interaction: MessageComponentInteraction,
  state: ComprehensiveWizardState,
  client: Client,
) {
...
async function handleTicketArchiveSelect(
  interaction: MessageComponentInteraction,
  state: ComprehensiveWizardState,
  client: Client,
) {
...
async function handleApplicationSystemStep(
  interaction: MessageComponentInteraction,
  state: ComprehensiveWizardState,
  client: Client,
) {
  // Check if application system is already configured
  const applicationAlreadyConfigured =
    state.existingConfigs?.applicationConfig && state.existingConfigs?.archivedApplicationConfig;
...
async function handleApplicationChannelSelect(
  interaction: MessageComponentInteraction,
  state: ComprehensiveWizardState,
  client: Client,
) {
...
async function handleApplicationCategorySelect(
  interaction: MessageComponentInteraction,
  state: ComprehensiveWizardState,
  client: Client,
) {
...
async function handleApplicationArchiveSelect(
  interaction: MessageComponentInteraction,
  state: ComprehensiveWizardState,
  client: Client,
) {
...
async function handleAnnouncementSystemStep(
  interaction: MessageComponentInteraction,
  state: ComprehensiveWizardState,
  client: Client,
) {
...
async function handleAnnouncementRoleSelect(
  interaction: MessageComponentInteraction,
  state: ComprehensiveWizardState,
  client: Client,
) {
...
async function handleAnnouncementChannelSelect(
  interaction: MessageComponentInteraction,
  state: ComprehensiveWizardState,
  client: Client,
) {
...
async function handleBaitChannelSystemStep(
  interaction: MessageComponentInteraction,
  state: ComprehensiveWizardState,
  client: Client,
) {
...
async function handleBaitChannelSelect(
  interaction: MessageComponentInteraction,
  state: ComprehensiveWizardState,
  client: Client,
) {
...
async function handleBaitChannelActionSelect(
  interaction: MessageComponentInteraction,
  state: ComprehensiveWizardState,
  client: Client,
) {
...
async function handleBaitChannelGracePeriodSelect(
  interaction: MessageComponentInteraction,
  state: ComprehensiveWizardState,
  client: Client,
) {
  ...
  collector.on('collect', async (selectInteraction: StringSelectMenuInteraction) => {
    ...
    await selectInteraction.deferUpdate();
    state.baitChannelConfig!.gracePeriodSeconds = parseInt(selectedGrace, 10);
    await handleBaitChannelLogSelect(selectInteraction, state, client);
    collector.stop();
  });
}

async function handleBaitChannelLogSelect(
  interaction: MessageComponentInteraction,
  state: ComprehensiveWizardState,
  client: Client,
) {
...
async function handleRoleManagementStep(
  interaction: MessageComponentInteraction,
  state: ComprehensiveWizardState,
  client: Client,
) {
  ...
  collector.on('end', async (_collected, reason) => {
    if (reason === 'time') {
      try {
        ...
        enhancedLogger.warn(
          lang.botSetup.logs.couldNotEditOnTimeout,
...
async function handleRoleTypeSelection(
  interaction: MessageComponentInteraction,
  state: ComprehensiveWizardState,
  client: Client,
) {
...
async function handleRoleSelection(
  ...
  collector.on('collect', async (roleInteraction: RoleSelectMenuInteraction) => {
    ...
    const modalSubmit = await roleInteraction
      .awaitModalSubmit({
    ...
    if (modalSubmit) {
      ...
      await handleAddMoreRoles(
        modalSubmit as unknown as MessageComponentInteraction,
        state,
        client,
      );
    }
...
async function handleAddMoreRoles(
  interaction: MessageComponentInteraction,
  state: ComprehensiveWizardState,
  client: Client,
) {
    ...
    if (buttonInteraction.customId === 'role_add_more') {
      ...
      try {
        ...
        enhancedLogger.error(
          lang.botSetup.errors.roleAddError,
          error as Error,
          LogCategory.COMMAND_EXECUTION,
        );
...
async function handleFinalSummary(
  ...
  const successEmbed = buildSuccessEmbed(state.config, {
    ...
    applicationConfigured:
      state.systemsConfigured.application || !!state.existingConfigs?.applicationConfig,
    announcementConfigured:
      state.systemsConfigured.announcement || !!state.existingConfigs?.announcementConfig,
    baitChannelConfigured:
      state.systemsConfigured.baitchannel || !!state.existingConfigs?.baitChannelConfig,
...
async function saveBotConfiguration(state: ComprehensiveWizardState) {
  try {
    ...
    let config = await botConfigRepo.findOne({
    ...
    enhancedLogger.error(
      lang.botSetup.errors.savingBotConfig,
      error as Error,
...
async function saveTicketConfiguration(state: ComprehensiveWizardState, client: Client) {
  ...
  try {
    ...
    // Setup archive forum
    const archiveForum = (await client.channels.fetch(
      state.ticketConfig.archiveForumId!,
    )) as ForumChannel;
    if (archiveForum) {
      ...
      try {
        ...
        enhancedLogger.warn(lang.botSetup.logs.couldNotPin, LogCategory.COMMAND_EXECUTION);
      }

      let archivedConfig = await archivedTicketConfigRepo.findOne({
        where: { guildId: state.guildId },
      });
      if (!archivedConfig) {
    ...
    enhancedLogger.error(
      lang.botSetup.errors.savingTicketConfig,
      error as Error,
      LogCategory.DATABASE,
    );
    throw error;
...
async function saveApplicationConfiguration(state: ComprehensiveWizardState, client: Client) {
  ...
  try {
    ...
    // Setup archive forum
    const archiveForum = (await client.channels.fetch(
      state.applicationConfig.archiveForumId!,
    )) as ForumChannel;
    if (archiveForum) {
      const msg = await archiveForum.threads.create({
      ...
      let archivedConfig = await archivedApplicationConfigRepo.findOne({
        where: { guildId: state.guildId },
      });
      if (!archivedConfig) {
    ...
    enhancedLogger.error(
      lang.botSetup.errors.savingApplicationConfig,
      error as Error,
      LogCategory.DATABASE,
    );
    throw error;
...
async function saveAnnouncementConfiguration(state: ComprehensiveWizardState, _client: Client) {
  ...
  try {
    ...
    let announcementConfig = await announcementConfigRepo.findOne({
      where: { guildId: state.guildId },
    });
...
async function saveBaitChannelConfiguration(state: ComprehensiveWizardState, client: Client) {
  ...
  try {
    ...
    let baitChannelConfig = await baitChannelConfigRepo.findOne({
      where: { guildId: state.guildId },
    });
    ...
    enhancedLogger.info(
      lang.botSetup.logs.baitChannelSaved
        .replace('{channelId}', baitChannelConfig.channelId)
        .replace('{enabled}', baitChannelConfig.enabled.toString()),
      LogCategory.COMMAND_EXECUTION,
    );

    // Send setup message to the bait channel
    const guild = await client.guilds.fetch(state.guildId);
    const baitChannel = (await guild.channels.fetch(
      state.baitChannelConfig.channelId!,
    )) as TextChannel;
    ...
    if (baitChannelManager) {
      ...
      enhancedLogger.info(
        lang.botSetup.logs.baitChannelCacheCleared,
        LogCategory.COMMAND_EXECUTION,
      );
    }
  } catch (error) {
    enhancedLogger.error(
      lang.botSetup.errors.savingBaitChannelConfig,
      error as Error,
      LogCategory.DATABASE,
    );
    throw error;
...
async function saveRoles(state: ComprehensiveWizardState) {
  ...
  try {
    ...
    for (const roleData of state.roles) {
      ...
      const existing = await savedRoleRepo.findOne({
        where: { guildId: state.guildId, role: roleData.role },
      });
    ...
    enhancedLogger.error(lang.botSetup.errors.savingRoles, error as Error, LogCategory.DATABASE);
    throw error;
  }
}
