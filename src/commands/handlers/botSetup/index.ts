...
import { buildRoleSelectionEmbed, buildRoleSelector } from './steps';
...
export async function botSetupHandler(
  ...
  if (!rateCheck.allowed) {
    ...
    await interaction.reply({
      embeds: [
        createErrorEmbed(lang.botSetup.rateLimit.exceeded.replace('{minutes}', minutes.toString())),
      ],
      flags: [MessageFlags.Ephemeral],
    });
  ...
  const archivedApplicationConfigRepository =
    AppDataSource.getRepository(ArchivedApplicationConfig);
...
async function handleEnhancedUpdateMode(
  ...
  const hasTicket =
    existingConfigs.existingTicketConfig && existingConfigs.existingArchivedTicketConfig;
  const hasApplication =
    existingConfigs.existingApplicationConfig && existingConfigs.existingArchivedApplicationConfig;
  ...
  statusLines.push(
    hasStaffRole
      ? lang.botSetup.update.staffRoleConfigured.replace(
          '{role}',
          existingConfigs.existingBotConfig!.globalStaffRole!,
        )
      : lang.botSetup.update.staffRoleNotConfigured,
  );

  statusLines.push(
    hasTicket ? lang.botSetup.update.ticketConfigured : lang.botSetup.update.ticketNotConfigured,
  );
  ...
  const updateEmbed = createInfoEmbed(lang.botSetup.update.title, statusLines.join('\n'));
  ...
  collector.on('collect', async buttonInteraction => {
    if (buttonInteraction.customId === 'update_reconfigure_all') {
      ...
      const partialConfigs = {
        ...
        existingArchivedTicketConfig: hasTicket
          ? existingConfigs.existingArchivedTicketConfig
          : undefined,
        existingApplicationConfig: hasApplication
          ? existingConfigs.existingApplicationConfig
          : undefined,
        existingArchivedApplicationConfig: hasApplication
          ? existingConfigs.existingArchivedApplicationConfig
          : undefined,
        existingAnnouncementConfig: hasAnnouncement
          ? existingConfigs.existingAnnouncementConfig
          : undefined,
        existingBaitChannelConfig: hasBaitChannel
