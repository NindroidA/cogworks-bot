  ...
  try {
    ...
    for (const summary of summaries) {
      if (summary.botConfigs !== 1) {
        console.log(
          `❌ FAIL: Guild ${summary.guildId} has ${summary.botConfigs} bot configs (expected 1)`,
        );
        passed = false;
      }
    }
    ...
    console.log(
      `Total Tickets:      ${summaries.reduce((sum, s) => sum + s.tickets, 0)}`,
    );
    console.log(
      `Total Applications: ${summaries.reduce((sum, s) => sum + s.applications, 0)}`,
    );
    console.log(
      `Total Positions:    ${summaries.reduce((sum, s) => sum + s.positions, 0)}`,
    );
    console.log(
      `Total Bait Logs:    ${summaries.reduce((sum, s) => sum + s.baitLogs, 0)}`,
    );
    console.log(
      `Total Saved Roles:  ${summaries.reduce((sum, s) => sum + s.savedRoles, 0)}`,
    );
  } catch (error) {
 ...
 */
async function checkForNullGuildIds(): Promise<{
  passed: boolean;
  ...
  const counts: Record<string, number> = {
    ...
    Applications: await applicationRepo.count({
      where: { guildId: null as any },
    }),
 ...
 */
async function checkForeignKeyIsolation(): Promise<{
  passed: boolean;
  message: string;
}> {
