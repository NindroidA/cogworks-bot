...
const fakeArchiveAndCloseApp = jest.fn(async () => ({ success: true, archived: true }));
...
const applicationRepoState: RepoState = { findOneByResult: null, updateCalls: [] };
const archivedAppConfigRepoState: RepoState = { findOneByResult: null, updateCalls: [] };
...
beforeEach(() => {
  ...
  fakeArchiveAndCloseApp.mockImplementation(async () => ({ success: true, archived: true }));
...
describe('POST /applications/:id/approve', () => {
  test('happy path: status flipped to accepted, approval message sent, audit logged', async () => {
    ...
    expect(fakeWriteAuditLog).toHaveBeenCalledWith('guild-1', 'application.approve', 'reviewer-99', { applicationId: 7 });
  });
  ...
  test('falls back to body.approvedBy when triggeredBy is absent', async () => {
    ...
    expect(fakeWriteAuditLog).toHaveBeenCalledWith('guild-1', 'application.approve', 'fallback-user', { applicationId: 7 });
  });
  ...
  test('returns 409 when application already closed', async () => {
    ...
    await expect(
      getRoute('POST /applications/:id/approve')('guild-1', { triggeredBy: 'r-1' }, '/applications/7/approve'),
    ).rejects.toMatchObject({ statusCode: 409, message: 'Application already closed' });
    expect(applicationRepoState.updateCalls).toHaveLength(0);
  });
  ...
  test('skips channel send when channel is not text-based', async () => {
    ...
    (fakeClient.channels.fetch as any).mockResolvedValue({ isTextBased: () => false });
...
describe('POST /applications/:id/deny', () => {
  test('happy path: status flipped to rejected, deny message includes reason', async () => {
    ...
    expect(result).toEqual({ success: true, applicationId: 7 });
    expect(applicationRepoState.updateCalls[0].partial).toEqual({ status: 'rejected' });
    expect(fakeChannelSend).toHaveBeenCalledTimes(1);
...
describe('POST /applications/:id/archive', () => {
  test('happy path: closes app, calls archiveAndCloseApplication, propagates archived flag', async () => {
    ...
    archivedAppConfigRepoState.findOneByResult = { guildId: 'guild-1', channelId: 'archive-forum-1' };
    ...
    expect(applicationRepoState.updateCalls[0].partial).toEqual({ status: 'closed' });
    expect(fakeArchiveAndCloseApp).toHaveBeenCalledTimes(1);
    expect(fakeArchiveAndCloseApp.mock.calls[0][4]).toBe('archive-forum-1');
    expect(fakeWriteAuditLog).toHaveBeenCalledWith('guild-1', 'application.archive', 'reviewer-99', { applicationId: 7 });
  });

  test('returns 404 when archive config missing', async () => {
    applicationRepoState.findOneByResult = { id: 7, guildId: 'guild-1', status: 'pending', channelId: 'c-1' };
    archivedAppConfigRepoState.findOneByResult = null;

    await expect(
      getRoute('POST /applications/:id/archive')('guild-1', {}, '/applications/7/archive'),
    ).rejects.toMatchObject({ statusCode: 404, message: 'Archive config not found' });
  ...
  test('archiveAndCloseApplication returns archived: false — handler propagates honest flag', async () => {
    applicationRepoState.findOneByResult = { id: 7, guildId: 'guild-1', status: 'pending', channelId: 'c-1' };
    archivedAppConfigRepoState.findOneByResult = { guildId: 'guild-1', channelId: 'archive-forum-1' };
    fakeArchiveAndCloseApp.mockResolvedValue({ success: true, archived: false });
