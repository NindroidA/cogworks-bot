...
describe("validateHexColor", () => {
  test("valid uppercase color returns valid", () => {
    expect(validateHexColor("#5865F2")).toEqual({ valid: true });
  });

  test("valid lowercase color returns valid", () => {
    expect(validateHexColor("#ff0000")).toEqual({ valid: true });
  });

  test("valid mixed case returns valid", () => {
  ...
  test("3-digit shorthand returns error", () => {
    const result = validateHexColor("#FFF");
    expect(result.valid).toBe(false);
    expect(result.error).toBeString();
  });

  test("invalid hex chars returns error", () => {
...
describe("optionalStringArray", () => {
  test("returns array of strings", () => {
    expect(optionalStringArray({ tags: ["a", "b"] }, "tags")).toEqual([
      "a",
      "b",
    ]);
  });
