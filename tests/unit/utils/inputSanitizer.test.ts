...
describe('validateSafeUrl', () => {
  ...
  test('should reject HTTP URL', () => {
    expect(validateSafeUrl('http://example.com')).toBe('Only HTTPS URLs are allowed');
  });
  ...
  test('should reject localhost', () => {
    expect(validateSafeUrl('https://localhost/admin')).toBe('Internal hostnames are not allowed');
  });

  test('should reject [::1]', () => {
    expect(validateSafeUrl('https://[::1]/admin')).toBe('Internal hostnames are not allowed');
  });

  test('should reject .local domains', () => {
    expect(validateSafeUrl('https://myserver.local/api')).toBe('Internal hostnames are not allowed');
  });

  test('should reject 10.x.x.x private IPs', () => {
    expect(validateSafeUrl('https://10.0.0.1')).toBe('Private/internal IP addresses are not allowed');
  });

  test('should reject 192.168.x.x private IPs', () => {
    expect(validateSafeUrl('https://192.168.1.1')).toBe('Private/internal IP addresses are not allowed');
  });

  test('should reject 172.16-31.x.x private IPs', () => {
    expect(validateSafeUrl('https://172.16.0.1')).toBe('Private/internal IP addresses are not allowed');
    expect(validateSafeUrl('https://172.31.255.255')).toBe('Private/internal IP addresses are not allowed');
  });
  ...
  test('should reject 127.x.x.x loopback', () => {
    expect(validateSafeUrl('https://127.0.0.1')).toBe('Private/internal IP addresses are not allowed');
  });

  test('should reject 169.254.x.x link-local', () => {
    expect(validateSafeUrl('https://169.254.1.1')).toBe('Private/internal IP addresses are not allowed');
  });

  test('should reject 0.0.0.0', () => {
    expect(validateSafeUrl('https://0.0.0.0')).toBe('Private/internal IP addresses are not allowed');
  });
  ...
  test('should reject IPv6 loopback in brackets', () => {
    expect(validateSafeUrl('https://[::1]')).toBe('Internal hostnames are not allowed');
  });

  test('should reject IPv6 link-local fe80:', () => {
    expect(validateSafeUrl('https://[fe80::1]')).toBe('Private/internal IP addresses are not allowed');
  });

  test('should reject IPv6 unique-local fc/fd', () => {
    expect(validateSafeUrl('https://[fc00::1]')).toBe('Private/internal IP addresses are not allowed');
    expect(validateSafeUrl('https://[fd00::1]')).toBe('Private/internal IP addresses are not allowed');
  });
});
...
describe('sanitizeMentions', () => {
  ...
  test('should preserve user mentions <@123456>', () => {
    expect(sanitizeMentions('<@123456789012345678>')).toBe('<@123456789012345678>');
  });
