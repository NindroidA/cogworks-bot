...
function makeManager(opts: {
  ...
  const pendingActionRepo = makeFakeRepo({
    findOneResult: null,
    findResults: [],
  });
  const keywordRepo =
    opts.withKeywordRepo === false ? undefined : makeFakeRepo(keywordState);
...
function makeRichMember(
  overrides: { id?: string; joinedTimestamp?: number | null } = {},
) {
  return {
...
describe("logAction (null-safety)", () => {
  test("writes log row using passed-in member when message.member is null", async () => {
    ...
    await (manager as any).logAction(
      message,
      member,
      "ban",
      {} as any,
